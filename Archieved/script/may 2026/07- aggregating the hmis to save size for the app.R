...
hmis <- readRDS("Archieved/data/processed/may 2026/upto_may_2026_hmis_for_app.rds")
...
# read sf
sf <- readRDS("Archieved/data/eth-admin3-v1082-clean-id.rds")|>
  sf::st_drop_geometry()
...
saveRDS(geo_lookup, "Archieved/data/processed/may 2026/geo_lookup.rds")

# create a population lookup
pop_lookup <- read_csv("Archieved/data/eth-admin3-v1082-clean-id_with_pop_2030.csv")|>
...
hmis_dashboard |>
  filter(data_type %in% c("tested", "positives") & month_year =="May-2026")|>
  group_by(data_type) |>
  summarise(total = sum(value, na.rm = T)) # correct

# saving the clean hmis for app use
saveRDS(hmis_dashboard, "Archieved/data/processed/may 2026/upto_may_2026_hmis_for_app.rds")
